...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None and jailPosition == ghostPosition:
            return 1.0
        if ((noisyDistance == None and jailPosition != ghostPosition) or 
        (noisyDistance != None and jailPosition == ghostPosition)):
            return 0.0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in positions:
            p = self.getObservationProb(observation, pacman_pos, pos, jail_pos)
            self.beliefs[pos] = p * self.beliefs[pos]
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in positions:
            positions_new[pos] = self.getPositionDistribution(gameState, pos)
        for pos in positions:
            total = 0
            for p in positions:
                total = (positions_new[p][pos] * self.beliefs[p]) + total
            belief[pos] = total
        self.beliefs = belief
        "*** END YOUR CODE HERE ***"
